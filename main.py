...
if rawcsv is not None:
    ...
    df["date"] = df["測定日"].dt.date - pd.to_timedelta(
        (df["測定日"].dt.time < pd.to_datetime("05:00:00").time()).astype(int), unit="d"
    )
    ...
    morning_data = (
        df_morning.groupby("date")
        .agg(
            {"最高血圧(mmHg)": "first", "最低血圧(mmHg)": "first", "脈拍(bpm)": "first"}
        )
        .reset_index()
    )

    evening_data = (
        df_evening.groupby("date")
        .agg(
            {"最高血圧(mmHg)": "first", "最低血圧(mmHg)": "first", "脈拍(bpm)": "first"}
        )
        .reset_index()
